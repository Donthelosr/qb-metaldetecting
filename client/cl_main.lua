...
RegisterNetEvent('qb-metaldetecting:startdetect', function(data)
    ...
    if inZone == 1 then 
        if not overheated then 
            local success = exports['qb-lock']:StartLockPickCircle(3,15, success)
            if success then
            QBCore.Functions.Progressbar('start_detect', 'Detecting...', Config.DetectTime, false, false, { -- Name | Label | Time | useWhileDead | canCancel
            ...
            QBCore.Functions.Notify('You sure you know what you are doing?', 'error', 5000)
        end
        else 
